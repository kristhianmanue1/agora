...
assert rows[:len(base_rows)]==base_rows and len(rows)-len(base_rows)<=7
assert {(r['case'],r['version']) for r in rows[len(base_rows):]}.issubset({(r['case'],r['version']) for r in protocol['pending']})
for name in ('continue.py','audit_continuation.py'):assert sha(b/name)==sha(out/name)
...
report=dict(artifact_integrity_verified=True,calls=len(rows),metrics=scores(gold,predictions),groups=groups,
...
report['responses_replayed']=sum('response' in json.loads(f.read_text()) for f in out.glob('v3/case-*/result.json'))
report['attempts_without_response']=len(rows)-report['responses_replayed']
report['new_calls']=len(rows)-len(base_rows)
report['all_slots_attempted']=len(rows)==8
