...
rows=json.loads((prior/'execution.json').read_text());attempted={(x['case'],x['version']) for x in rows};assert len(rows)==1 and len(attempted)==1
cases=json.loads((prior/'inputs/cases.json').read_text());pending=[]
for i,c in enumerate(cases):
 for version in ['v3']:
  if (c['id'],version) not in attempted:pending.append((c,version))
assert len(pending)==7 and ('case-01','v3') in attempted
...
protocol={'base':'glm-01','base_files_sha256':prior_hashes,'max_new_calls':7,'max_new_reserved_output_tokens':57344,
 'pending':[{'case':c['id'],'version':v} for c,v in pending],'no_retries':True,'unknown_outcome_preserved':'v3/case-01',
...
for c,version in pending:
 ...
 try:
  p=subprocess.run([sys.executable,str(out/'worker.py'),str(out/'inputs'/c['candidate']),str(dest),version,str(out/'transport.py')],env=env,timeout=200);code=p.returncode
