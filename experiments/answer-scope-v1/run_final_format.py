"""Eight producer-known development cases; one request each, no retries."""
from pathlib import Path
import hashlib,json,os,shutil,subprocess,sys,time

def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def main():
 b=Path(__file__).resolve().parent;root=b.parents[1];out=b/'runs/glm-04';out.mkdir(parents=True,exist_ok=False)
 shutil.copytree(root/'src/agora',out/'code/agora',ignore=shutil.ignore_patterns('__pycache__'))
 shutil.copytree(b/'inputs/final',out/'inputs');shutil.copyfile(__file__,out/'run.py')
 protocol={'model':'glm-5.3-flash','profile':'concise-v1','max_calls':8,'max_output_tokens':8192,'temperature':0,'socket_seconds':90,'process_seconds':110,'retries':0,'holdout':False,'purpose':'development; not QASPER re-evaluation','stop':'non-timeout operational failure'}
