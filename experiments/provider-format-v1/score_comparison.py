...
def main():
 root=Path(__file__).resolve().parents[2];out=Path(__file__).resolve().parent/'runs/paired-01'
 ...
 result={'scores':scores,'details':details,'campaign_complete':len(execution)==16,'integrity_verified':True,'paired_prompts_identical':True,'semantic_review':'pending; expected status match is not semantic fidelity'}
