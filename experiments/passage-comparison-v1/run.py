...
def main():
    ...
    for name in ('source.txt', 'cases.json', 'protocol.md', 'run.py'):
    ...
    revision = digest(out / 'source.txt')
    ...
    for case in cases:
        for mode in ('source', 'reference', 'retrieve'):
            ...
            command=[sys.executable,'-m','agora.passage_cli','--source',str(out/'source.txt'),
                '--source-id','synthetic-comparison-v1','--sha256',revision,'--question',case['question'],
                '--mode',mode,'--content-budget','20480' if mode=='source' else '1800',
                '--prompt-budget','65536','--max-output-tokens','2048','--timeout-seconds','90',
                '--model','glm-5.3-flash','--out',str(dest)]
            if mode=='reference':
                for start,end in case['ranges']:command+=['--range',f'{start}:{end}']
