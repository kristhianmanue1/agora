...
import hashlib,json
b=Path(__file__).resolve().parent;p=b/'runs/glm-01'
...
result={'integrity_verified':True,'attempted_planned_calls':len(rows)==4,'all_calls_completed':all(x['status']=='complete' for x in rows),'metrics':metrics,'known_tokens':sum(x['tokens'] for x in rows if x['tokens'] is not None),'unknown_usage_calls':sum(x['tokens'] is None for x in rows),'seconds':round(sum(x['seconds'] for x in rows),3),'semantic_review':'separate producer review required'}
