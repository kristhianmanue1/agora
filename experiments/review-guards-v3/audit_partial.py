...
report['audit_extension']='Partial-run handling added after timeout; frozen original audit unchanged. Metrics and expectations unchanged.'
report['audit_extension_sha256']=sha(Path(__file__))
(out/'audit-partial.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
