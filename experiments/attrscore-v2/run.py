"""At most 48 calls, no retries; freeze inputs, code, scoring and protocol first."""
...
shutil.copyfile(root/'docs/piloto-attrscore-02.md', out/'protocol.md')
protocol = {'max_calls':48,'model':'glm-5.3-flash','temperature':0,'max_output_tokens':8192,
    'max_reserved_output_tokens':393216,'http_and_transport_timeout_seconds':180,'worker_timeout_seconds':200,
    ...
    'metrics':'original gold three-class accuracy and macro F1; failures and missing retained; descriptive relevance/action counts, usage/time; no new semantic gold',
    'limits':'24-case stratified external subset, not official benchmark score; no authentication or Spanish/long-document claims'}
...
cases=json.loads((out/'inputs/pilot-02/cases.json').read_text())
assert len(cases)==24
for i,c in enumerate(cases):
    for version in (['v1','v2'] if i%2==0 else ['v2','v1']):
        ...
        try:
            p=subprocess.run([sys.executable,str(out/'worker.py'),str(out/'inputs/pilot-02'/c['candidate']),str(dest),version,str(out/'transport.py')],env=env,timeout=200)
            code=p.returncode
