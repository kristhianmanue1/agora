"""Explicit GLM transport for the experimental budget-routed multipart consumer."""
import argparse
import json
import os
...
from .routed_query import query_routed
...
def main():
    ...
    parser.add_argument('--full-source-budget', type=int, default=32768)
    parser.add_argument('--traversal', type=Path)
    parser.add_argument('--traversal-sha256')
    ...
    parser.add_argument('--answer-profile', choices=('legacy', 'concise-v1'), default='legacy')
    parser.add_argument('--response-format', choices=('default', 'json_object'), default='default',
                        help='Optional JSON mode; does not enforce the response schema')
    ...
    attempts = 0
    modules = ("passages.py", "routed_cli.py", "routed_query.py", "traversal_query.py", "traversal.py", "source.py", "query.py", "transform.py", "__main__.py")
    ...
    try:
        ...
        with args.parts.open('rb') as stream:
            parts_data=stream.read(32769)
        if len(parts_data)>32768:raise ValueError('parts_file_too_large')
        parts=json.loads(parts_data)
        record_data=None
        if args.traversal is not None:
            with args.traversal.open('rb') as stream:record_data=stream.read(8388609)
        result=query_routed(adapter,args.sha256,args.question,parts,provider,
                            full_source_budget=args.full_source_budget,prompt_budget=args.prompt_budget,
                            traversal_data=record_data,traversal_hash=args.traversal_sha256,
                            response_language=args.response_language,answer_profile=args.answer_profile)
